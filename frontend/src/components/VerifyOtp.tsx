...
const VerifyOtp = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await axios.post(`${user_service}/api/v1/auth/${endpoint}/verify`, {
           email,
           otp: otpString,
        },
       {
  ...
  const handleResendOtp = async () => {
    ...
    setError("");
    try {
      const { data } = await axios.post(`${user_service}/api/v1/auth/login`, {
        email,
      });
      ...
      <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
        <div className="max-w-md w-full">
          <div className="bg-gray-800 border border-gray-700 rounded-lg p-8 text-center">
            ...
            <p className="text-gray-300 mb-6">Missing required parameters. Please try logging in again.</p>
            <button
